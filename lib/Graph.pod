...
vertices ignored (since they could be renamed arbitrarily.)  Note that for
large graphs the product of these factorials can overflow the maximum
presentable number (the floating point number) in the computer and you
might get for example I<Infinity> as the result.
...
Roughly, the above numbers mean that in a megabyte you can fit
for example a graph of about 1000 vertices and about 2500 edges.
...
horribly (or at least will look funny) with hyperthingies.
