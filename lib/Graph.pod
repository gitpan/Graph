...
The graph I<$h> is not affected.  By default duplicate vertices
and edges are ignored, to change this behaviour use the C<resolve>
option: it can be a code reference called with either
...
or a boolean value.  The return value is tested: if it
...
The graph I<$h> is not affected.  By default missing vertices
and edges are ignored, to change this behaviour use the C<resolve>
option: it can be a code reference called with either
...
Return the vertices of a cycle.
...
B<In future releases of Graph (any release after 0.50) the 0.2xxx
...
the Graph 0.2xxx backward compatibility API.
...
Thanks to Nathan Goodman and Andras Solomon for bravely betatesting my
0.5xxx code.  If they missed something, that was only because of my
fiendish code.
