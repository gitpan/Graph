...
This module is not for B<drawing> any sort of I<graphics>, but instead
it is for creating abstract data structures called graphs, and for
doing various kinds of operations on those.
...
see L</copy>.  (Uses Data::Dumper behind the scenes.)
...
A complete graph has an edge (v,u) for every original edge (u,v).
...
Add the vertices and edges of the graph I<$h> to the graph I<$g>.
The graph I<$h> is not affected.  By default already existing vertices
and edges are silently accepted, to change this behaviour use the
C<resolve> option: it can be a code reference that gets called
(when there a corresponding edge or vertex already exists) with either
...
or a boolean value.  The return value of C<resolve> is tested: if it
is true, the vertex or the edge is added, if false, not.

If you want to add only the vertices or the edges,
use the C<vertices> and C<edges> options:

    $g->add_graph($h, vertices => 0)
    $g->add_graph($h, egdes    => 0)
...
Delete the vertices and edges of the graph I<$h> from the graph I<$g>.
The graph I<$h> is not affected.  By default already missing vertices
and edges are silently accepted, to change this behaviour use the
C<resolve> option: it can be a code reference that gets called (when
there is no corresponding edge or vertex to delete) with either
...
or a boolean value.  The return value of C<resolve> is tested: if it
is true, the vertex or the edge is deleted, if false, not.

If you want to delete only the vertices or the edges,
use the C<vertices> and C<edges> options:

    $g->delete_graph($h, vertices => 0)
    $g->delete_graph($h, egdes    => 0)
...
an equal number of deletions to become completely deleteted.
...
you can call the stringification as the method
...
See also L</connected_components>, L<connected_component>, and
L<same_connected_components>, and L</biconnectivity>.
...
graph as a list of anonymous arrays of vertices in the components. The
ordering of the anonymous arrays or the ordering of the vertices
inside the anonymous arrays is undefined.  Also note that one vertex
...
component.  The ordering of the vertices inside the anonymous array is
...
component, the indexing starting from zero.  The order of vertices is
undefined, while the order of the connected components is the as from
...
=item weakly_connected_component_by_vertex
    ...
    $i = $g->weakly_connected_component($v)
...
component, the indexing starting zero.  The order of vertices is
undefined, while the order of the weakly connected components is
same as from weakly_connected_components().
...
Fof a directed graph, return the strongly connected graph of the
graph.  The vertices in the strongly connected graph are formed from
the strongly connected elements of the graph (currently by sorting the
vertices of the original graph present in the strongly connected
component and concatenating the vertex names with C<+>, for example
C<a+b+c>); the edges connect the strongly connected elements of the
graph.
...
L</is_sink_vertex> for the definition of a sink vertex.
...
L</is_source_vertex> for the definition of a source vertex.
...
L</is_isolated_vertex> for the definition of an isolated vertex.
...
L</is_interior_vertex> for the definition of an interior vertex.
...
L</is_exterior_vertex> for the definition of an exterior vertex.
...
L</is_self_loop_vertex> for the definition of a self-loop vertex.
...
The in-degree minus the out-degree at the vertex.
...
a count on it: add operations increase the count, and delete operations
decrease the count, and once the count goes to zero, the edge or vertex
is deleted.  You can think of this as the graph elements being I<refcounted>,
if that sounds more familiar.
...
To enable countededges on a graph, give the C<countedged> parameter a
true value
...
A multiedged (either the weak or the strong kind) graph is a I<multigraph>,
for which you can test with C<is_multi_graph()>.

B<Note>: The various graph algorithms do not in general work well
with multigraphs, and no effort has been made to test the algorithms
with multigraphs.
...
=back
    ...
    $id0 = $g->add_vertex_get_id($v)
    $id1 = $g->add_edge_get_id($u, $v)
...
The graph cannot contain negative edges (negative edges cause the
algorithm to abort).
...
Find the single-source shortest paths using Bellman-Ford's algorithm.
The graph can contain negative edges but not negative cycles (negative
cycles cause the algorithm to abort).
...
using Floyd-Warshall's algorithm.  The length is the sum of weight
attribute of the edges along the shortest path.  If no weight
...
the graph.  Note that this recomputes the APSP each time when called.
...
along the shortest path.  Note that this recomputes the APSP each
time when called.
...
In scalar context return the shortest (shortest) length over all the
vertex pairs of the graph.  In list context return the vertices
along the shortest path.  Note that this recomputes the APSP each
time when called.
...
The number of shortest paths is returned (this should be equal to
V*V).  The $callback is a sub reference that receives four arguments:
...
Return a random predeceessor of the vertex in the graph, or undef if there
are no predeceessors.
...
=item set_edge_attributes
    ...
    $g->get_vertex_attributes($u, $v, $attr)
...
=item set_graph_attribute
    ...
    $g->set_graph_attribute($u, $v, $name, $value)
...
=item add_weighted_vertices
    ...
    $g->add_weighted_vertices($u, $v, ..., $weight)
...
than brute force wading through of all possibilities are known.
...
conducted.  It is basically the product of the factorials of the absolute
values of in-degrees and out-degree pairs at each vertex, with the isolated
vertices ignored (since they could be renamed arbitrarily.)
...
These size approximations are very approximate and optimistic
...
In practice you can expect at least about 200 bytes per vertex,
and twice that for an edge.  The numbers are for a 32-bit platform.
...
B<NOTE 1>: most usual graph algorithms (and basic concepts) break
horribly (or at least look funny) with hyperthingies.
...
That's right, vertices can be omni, too - and that is indeed the default.
You can turn it off and then $g->add_vertex(qw(a b)) no more means adding
also a vertex qw(b a).
...
participating in edges and vertices.  Without too much explanation:
...
Return the average (edge) degree taken over all vertices.
...
Maybe you have code based on Graph 0.2xxxx that assumes Graphs being
blessed hash references, possibly also assuming that certain hash keys
are available to use for your own purposes.  In Graph 0.50 none of this
is true.  Please do not expect any particular internal implementation
of Graphs.  Use inheritance and graph attributes.
