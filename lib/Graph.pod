...
For a directed graph, return the strongly connected graph of the
...
The result weights can be retrieved from the result graph by

	my $w = $sptg->get_vertex_attribute($v, 'weight');

You can choose the first vertex of the result with $opt{ first_root },
otherwise a random vertex is chosen.
...
You can choose the first vertex with $opt{ first_root }, otherwise
...
along the shortest path.  Note that this recomputes the APSP each time
when called.
